...
function statusFields(soption, status, procedure)
{
        ...
        {
            $("input[name=prev_rev_month]").prop('disabled', status);
            // ESTADO 30 - FORMS INITIALS 
            if(procedure == "is_reconnection" )
            {
            ...
            else if(procedure == "is_new_connection")
            {
                $("#owner_id").prop('disabled', status);
                $("#concelho").prop('disabled', status);
                $("#freg").prop('disabled', status);
...
function FormChgOverReconnection(action, changeover, reconnection, con_type, soption)
{
    ...
    if(valor_rec_chg_over == "is_reconnection" || reconnection==1)
    {
        ...
        statusFields(soption, true,"is_reconnection");
    }
    ...
    else if(valor_rec_chg_over == "is_changeover" || changeover==1)
    {
        ...
        statusFields(soption, false,"is_changeover");
    ...
    else if(valor_rec_chg_over == "is_new_connection" || reconnection==0 || changeover==0)
    {
        ...
        statusFields(soption, false,"is_new_connection");
    }
...
function equipConnectionAssoc(equip_id, prop_id, conn_id)
{
    ...
    $.ajax({ method: "GET", url: "webservice.php", data: { 'equip_connection_assoc': '1', 'equip_id': equip_id, 'prop_id': prop_id, 'conn_id': conn_id}})
        .done(function( data )
        {
            ...
            console.log(servico_des,modelo,equip,is_pon,is_olt,is_antenna);
            if(servico_des == 0 || modelo == 0 || equip == 0 || is_pon==0 || is_olt==0 || is_antenna==0)
            {
...
function addClientNew()
{
    ...
    if(cont % 2 == 0)
    {
        $("#add_client_but").html("Active 'Add New Client'");
    ...
    {
        // MOSTRAR O FORMUALRIO DE ADICIONAR CLIENTES NAS LEADS DO ESTADO 50
        $("#add_client_but").html("Deactive 'Add New Client'");
...
// LEADS.PHP - ESTADO 30
// ?propleads=1&lead_id=9355
function NewCustomerState30(lead_id)
{
    // NOME DO UTILIZADOR
    var localuser = $("#localuser_username").html();
    // NOME DO CLIENTE
    var salut = $("select[name=salut_cust]").val();
    var name = $("input[name=name_cust]").val();
    // EMAIL, & ENDERECO
    var address = $("input[name=address_cust]").val();
    var email = $("input[name=email_cust]").val();
    // TELEFONE
    var telef = $("input[name=telef_cust]").val();
    // NUMERO FISCAL
    var fiscal_nr = $("input[name=fiscal_nr_cust]").val();
    // LINGUA
    var lang = $("select[name=lang_cust]").val();

    // NOTAS
    var notes = $("input[name=notes_cust]").val();

    // OUTRAS OPCOES
    var is_commercial = $("input[name=is_commercial_cust]:checked").length;
    var is_management = $("input[name=is_management_cust]:checked").length;
    var is_agent = $("input[name=is_agent_cust]:checked").length;
    ...
    $.ajax({
        ...
        success: (function (data) {
            ...
            {
                $("#warn_submit_cust").html(result['msg']);
            }
            // SUBMISSOAO DO FORMULARIO DE ADICIONAR CLIENTE FEITO COM SUCESSO
            else if(result['msg'] == '' && result['error'] == '' && result['succ'] != '')
            {
                $("#warn_submit_cust").html(result['succ']);
                // APAGAR OS DADOS
                $("input[name=name_cust]").val('');
                $("input[name=address_cust]").val('');
                $("input[name=email_cust]").val('');
                $("input[name=telef_cust]").val('');
                $("input[name=fiscal_nr_cust]").val('');
                // DESATIVAR AS CHECKBOXES
                $("input[name=is_commercial_cust]").prop("checked", false);
                $("input[name=is_management_cust]").prop("checked", false);
                $("input[name=is_agent_cust]").prop("checked", false);

                $("input[name=notes_cust]").val('');

                $("#fiscal_num_warn").html('');
            ...
            {
                $("#warn_submit_cust").html(result['error']);
            }
        })
...
var d = 0;
// VERIFICAR O NUMERO DE FISCAL - LEADS.PHP - ESTADO 30
// ?propleads=1&lead_id=9355
function checkFiscalNumber(num_fiscal)
{
    ...
    if(num_fiscal != "")
    {
        ...
        $.ajax({ method: "GET", url: "webservice.php", data: { 'num_fiscal_customer': '1', 'num_fiscal': num_fiscal}})
            .done(function( data )
            {
                ...
                {
                    $("#divfiscalnumber").html("<b>Fiscal Number:</b> <font color=red>*</font>");
                    $("#fiscal_num_warn").html("<font color=red>The Fiscal Number must be a number</font>");
                }
                else
                {
                    ...
                    {
                        $("#divfiscalnumber").html("<b>Fiscal Number:</b> <font color=green>*</font>");
    ...
    else
    {
        ...
        $("#fiscal_num_warn").html('<font color=red>Missing Fiscal Number</font><br>');
        //$("#fiscal_num_warn").html("<br>"+result['msg']);
        //fiscal_valid -= 1;
        fiscal_valid = 0;
        d = fiscal_valid;
...
// VALIDAR OS CAMPOS DO FORMULARIO DE ADICIONAR CLIENTE NO CASO DE FOR UMA RECONNECTION - LEAD.PHP - ESTADO 30
// ?propleads=1&lead_id=9355
function validateCustForm()
{
    ...
    // ENDERECO
    var address_cust = $("#address_cust").val();
    // EMAIL
    var email = $("#email_cust").val();
    // TELEFONE
    var telef = $("#telef_cust").val();
    // NUMERO FISCAL
    var fiscal_nr = $("#fiscal_nr_cust").val();

    // se nao tem o nome do cliente
    if (name_cust == "")
    {
        $("#divname").html("<b>Name:</b> <font color=red>*</font>");
        //act_button_cust -= 1;
    }
    else
    {
        $("#divname").html("<b>Name:</b> <font color=green>*</font>");
        formready += 1;
    }
    ...
    if (address_cust == "")
    {
        $("#divbillingaddr").html("<b>Billing Address:</b> <font color=red>*</font>");
        //act_button_cust -= 1;
    }
    else
    {
        $("#divbillingaddr").html("<b>Billing Address:</b> <font color=green>*</font>");
        formready += 1;
    }
    ...
    if(email == "")
    {
        $("#divemailcust").html("<b>Email:</b> <font color=red>*</font>");
        //act_button_cust -= 1;
    }
    else
    {
        ...
        {
            $("#divemailcust").html("<b>Email:</b> <font color=green>*</font>");
            formready += 1;
        }
    }
    ...
    else
    {
        ...
        else
        {
            ...
            var fiscal_nr_length = fiscal_nr.length;
            if(fiscal_nr_length != 9)
            {
                $("#divfiscalnumber").html("<b>Fiscal Number:</b> <font color=red>*</font>");
                //act_button_cust -= 1;
            }
    ...
    if(telef == "")
    {
        ...
        $("#divphone").html("<b>Phone:</b> <font color=red>*</font>");
        //act_button_cust -= 1;
    }
    else
    {
        ...
        {
            $("#divphone").html("<b>Phone:</b> <font color=green>*</font>");
            formready += 1;
        }
    ...
    if(s == 5)
    {
        $("#new_cust").prop('disabled', false);
    }
    else
    {
        $("#new_cust").prop('disabled', true);
    }
...
function subscriber_lead_sub(lead_sub)
{
    $.ajax({ method: "GET", url: "webservice.php", data: { 'subscriber_list': '1'}})
        .done(function( data )
        {
                var result = $.parseJSON(data);
                var owner_html = '';
                    ...
                    {
                        owner_html += '<option value="' + result[i][1] + '">' + result[i][0] + '</option>';
                    }
...
function subscriber_lead_sub_chg_over(lead_sub)
{
    $.ajax({ method: "GET", url: "webservice.php", data: { 'subscriber_list': '1'}})
        .done(function( data )
        {
                ...
                var owner_html = '';
                    ...
                    $("#owner_id").html(owner_html);
                    $("#owner_id").val(lead_sub);
        });
}
...
function RecPropConnType(prop_id,con_type,conn_id,lead_sub,checked)
{
    ...
    if(lead_sub != 0 || lead_sub != "")
    {
        subscriber_lead_sub_chg_over(lead_sub);
    }
}
...
{
    $.ajax({ method: "GET", url: "webservice.php", data: { 'subscriber_prop_id': '1', 'prop_id': prop_id}})
        .done(function( data )
        {
            var result = $.parseJSON(data);
