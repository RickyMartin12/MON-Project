...
(function($) {
   ...
   class BodModal {
      ...
      openModal(evt) {
         ...
         //$('body').on('click touchstart', this.closeModal);

         
         var closeModal_popeup = this;
         


         $('body').on('keydown touchstart', function(e) {
            if(e.keyCode==27){
               closeModal_popeup.closeModal();
               //closeModal_popeup.clear();
            }
            else if(e.keyCode==37){
               closeModal_popeup.prevSlide();
               //closeModal_popeup.clear();
            }
            else if(e.keyCode==39){
               closeModal_popeup.nextSlide();
               //closeModal_popeup.clear();
            }
        });
      ...
      buildModalHtml(filename) {
         ...
         this.$modal.find('img').each(function() {
            ...
            let hrefFile = $(this).parent().data('link_page');
            if(imageHref.match(".pdf") && !imageHref.match("_pdf") && !imageHref.match(".png") && !imageHref.match(".jpg"))
            {
               imageHref = $(this).attr("src");
            }
            
            let curFilename = that.constructor.extractFile(imageHref);
            if (curFilename === filename) {
               html += "<div class='bodModal bodActiveSlide' style=\"position: relative\">";
               that.activeSlide = that.slideCount;
            } else {
               html += "<div class='bodModal' style=\"position: relative\">";
            }


            

            html += "<a href='"+hrefFile+"' target=_blank><img "
            html += "src='" + imageHref + "' alt='" + imageAlt + "' ></a>";
            // output title
