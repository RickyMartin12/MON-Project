...
include MON_ROOT."/includes/functions.php";
...
setTimeout(function() {
  location.href='kpi.php?page=<?php echo ($page+1);?>';
}, 30000);
 </script>
<?php } ?>
...
<body width=1980px height=1080px bgcolor=#dddddd>
...
echo"
<table  width=1980px  height=1080px bgcolor=#ffffff>
<tr><td width=45% align=center> <h2> Network KPIs</h2> 
...
<table>
<tr><th>area<th>total connections<th> YtD
";
$zones=array();
...
{
	echo "<tr><td>".$zone . "<td>". $number ."<td>".$zonesYTD[$zone];
	$tot1+=$number;
	$tot2+=$zonesYTD[$zone];
}

echo "
<tr><td><b>totals:</b><td>$tot1<td>$tot2
</table>
<br><br>
";
...
/*
echo "<table><tr><td>sales per day YtD
";
for($i=1;$i<32;$i++)
{
$j= str_pad($i, 2, "0", STR_PAD_LEFT);
$contracts=$mon3->query("select count(id) from property_leads where date_papwk LIKE \"2019-10-$j\" ")->fetch_assoc();
$total+=$contracts['count(id)'];
echo "<tr><td> $i<td>".$contracts['count(id)'];
...
//$kpis=$mon3->query("select distinct(kpi) from kpis");
//while($kpi=$kpis->fetch_assoc())
...
echo "<tr><td>".$kpi;
$values=$mon3->query("select month,value from kpis where kpi=\"".$kpi."\" and 
( month=\"$month_3\" or month=\"$month_2\" or month=\"$month_1\" ) order by month");
...
$installs_in=$mon3->query("select date_papwk,date_installed from property_leads where date_installed like \"".$month."%\"
		and date_papwk!=\"\" ");
	echo $mon->error;
...
<div id=thermo>
  <div>
  	<img id=thermogif src=\"includes/thermo.php?goal=4000&startup=3000&current=".$subs."\">
</div>
</table>
";
...
echo " <td width=45% align=center > <h2> Installations KPIs </h2>

<table  height=1000px width=100%> 
<tr><td colspan=2 width=45%> <h3>Stats for ".date("M-Y")."</h3> <td width=10%> <td colspan=2 width=45%> <h3>Stats for ".date("Y")."</h3>
<tr><td width=25%>total leads in<td width=20%> ".$leads_m['count(id)']." <td width=10%><td width=25%>total leads in<td width=20%>".$leads_y['count(id)']."
<tr><td>total contracts in<td>".$contrs_m['count(id)']." <td><td>total contracts in<td>".$contrs_y['count(id)']."
<tr><td>total connections<td>".$cons_m['count(id)']." <td><td>total connections<td> ".$cons_y['count(id)']."
<tr><td colspan=5><br>
...
<tr><td > Fibre homes active: <td > ".$fibrecust->num_rows ." <td><td >Coax homes active: <td w> ".$coaxcust->num_rows ." 
<tr><td > Fibre homes connected: <td> ".$tfinalconns->num_rows ."<td><td >Coax homes connected: <td > ".$tfinalconc->num_rows ."

<tr><td colspan=5 align=center height=200px>
